...
Cada individuo se asocia a un \textit{seed} de origen; la reproducción ocurre dentro del mismo \textit{cluster} (sin cruzamiento entre cuencas distintas) y cada hijo se proyecta de vuelta a la caja. Por continuidad física, perturbaciones pequeñas ($\leq 20$~nm) de un enfocador conocido tienen $R^2$ cercano, lo que garantiza que los candidatos permanezcan en la región fiable del surrogate. El radio $\delta$ se redujo progresivamente entre iteraciones (20~nm $\rightarrow$ 10~nm $\rightarrow$ 5~nm) para refinar la búsqueda. La Figura~\ref{fig:manifold_esquema} ilustra la idea: un optimizador libre escapa hacia las regiones de extrapolación donde el surrogate sobreestima, mientras que la caja $\ell_\infty(\delta)$ mantiene la búsqueda en el vecindario validado.

\begin{figure}[H]
    \centering
    \resizebox{0.85\textwidth}{!}{\input{Figuras/manifold_esquema.tikz}}
    ...
    $\ell_\infty(\delta)$ alrededor de un \textit{seed} confirmado.}
...
\textit{cluster} (un número fijo por cada \textit{seed}), evitando que todos converjan a una
sola cuenca. La Figura~\ref{fig:ga_esquema} resume el flujo completo de este GA particular.

\begin{figure}[H]
    \centering
    \input{Figuras/ga_esquema.tikz}
    \caption{Estructura del algoritmo genético restringido al manifold. A partir de un conjunto de \textit{seeds} confirmados por FDTD, la población se inicializa y se mantiene en una caja $\ell_\infty(\delta)$ por \textit{cluster}. Cada generación evalúa la aptitud combinada con el surrogate, selecciona por torneo \emph{dentro} de cada \textit{cluster}, aplica cruzamiento y mutación, y proyecta de vuelta a la caja (con elitismo); el lazo se repite $n$ generaciones. La selección final, diversificada por \textit{cluster}, alimenta la confirmación FDTD. Las dos diferencias respecto a un GA estándar, la caja por
    \textit{cluster} y la aptitud combinada, son los dos pilares del re-enfoque.}
    \label{fig:ga_esquema}
\end{figure}
...
Este procedimiento cumple un doble propósito: refinar la mejor geometría sin el sesgo del surrogate, y caracterizar la robustez local de cada cuenca. Esto es, qué tan sensible es el $R^2$ a perturbaciones del orden de las tolerancias de fabricación. Los resultados de este análisis, que revelaron diferencias cualitativas entre cuencas, se presentan en la Sección~\ref{sec:resultados}.
