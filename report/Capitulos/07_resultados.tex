...
Se ejecutaron cinco iteraciones de optimización; cuatro de ellas tuvieron confirmación FDTD independiente. La Tabla~\ref{tab:iteraciones} resume su evolución, que ilustra el efecto de restringir progresivamente la búsqueda al manifold de geometrías validadas.
...
\begin{figure}[H]
    ...
    \caption{Predicción del surrogate ($\hat{R}^2_{\text{pred}}$) frente al valor real confirmado por FDTD ($R^2_{\text{real}}$) para los candidatos de las iteraciones 3--5. Los puntos por encima de la diagonal evidencian la sobreestimación del surrogate en la zona alta, que motivó la restricción al manifold.}
    \label{fig:pred_vs_real}
\end{figure}

La iteración~1 (GA libre) confirmó el diagnóstico de la Sección~\ref{subsec:ciclo_activo}: todos los candidatos colapsaron a $R^2 \approx 0.5$ con una brecha media de $+0.48$ respecto a lo predicho. Al restringir al manifold (iter.~3--5), los candidatos se acercaron sistemáticamente al umbral: la iteración~4 produjo el primer cruce, \texttt{mvp\_c09} con $R^2_{\text{real}} = 0.9025$. La iteración~5, con la caja más estrecha ($5$~nm), no superó a la~4; de hecho, la mayoría de los candidatos derivados quedó por debajo de su semilla, indicando que las cuencas exploradas ya estaban cerca de su óptimo local.

\subsubsection{El Estancamiento de las Iteraciones}

Tres iteraciones con cajas decrecientes ($20 \rightarrow 10 \rightarrow 5$~nm) y tres versiones del surrogate convergieron al mismo rango $[0.85, 0.91]$, sin que reducir la caja mejorara el resultado. Este estancamiento se mostró robusto al surrogate, al radio de búsqueda y a las semillas, lo que sugería que el límite reflejaba una propiedad local del paisaje físico más que un mero defecto del modelo, una hipótesis que el análisis de cuencas confirmaría.

% --------------------------------------------------
\subsection{Convergencia del Pipeline: Calidad Global de los Candidatos}\label{subsec:convergencia}

Más allá del puñado de geometrías que cruzan el umbral $R^2 \geq 0.90$, el resultado agregado más informativo es el desplazamiento de toda la distribución de calidad de los candidatos respecto al conjunto histórico consolidado. La Tabla~\ref{tab:convergencia} compara la distribución de $R^2_{\text{real}}$ del dataset consolidado con la de los candidatos producidos por el enfoque definitivo: el GA restringido al manifold (iteraciones 3--5) y el hill-climbing sobre la cuenca robusta.
...
\midrule
Dataset consolidado                          & 1{,}110 & 0.24 & 11\,\% & 2.1\,\% & 0.4\,\% \\
GA restringido al manifold (iter.\ 3--5)     & 30      & 0.79 & 87\,\% & 60\,\%  & 3\,\%   \\
Hill-climbing cuenca robusta (627)           & 50      & 0.73 & 92\,\% & 18\,\%  & 6\,\%   \\
\bottomrule
\end{tabular}
\caption{Distribución de $R^2_{\text{real}}$ de los candidatos del enfoque definitivo frente al dataset consolidado. El enfoque definitivo concentra los candidatos en la región de alta calidad, desplazando la media de $0.24$ a ${\sim}0.79$.}
\label{tab:convergencia}
\end{table}

La diferencia respecto al conjunto consolidado es notable. En este, solo el $11\%$ de las geometrías alcanza $R^2_{\text{real}} \geq 0.60$, con una media de $0.24$; los candidatos del GA restringido al manifold, en cambio, se concentran en la región de alta calidad: el $87\%$ supera $0.60$, el $60\%$ supera $0.85$ (frente a apenas $2.1\%$ en el conjunto consolidado) y su $R^2$ medio es $0.79$, más del triple de la media histórica. El hill-climbing sobre la cuenca robusta sostiene esa concentración ($92\%$ por encima de $0.60$). En total, el enfoque definitivo produjo cuatro geometrías con $R^2_{\text{real}} \geq 0.90$ (una del GA y tres del hill-climbing).

Este desplazamiento es, por sí mismo, uno de los resultados más relevantes del proyecto: muestra que el pipeline no halló sus mejores diseños por azar, sino que combinó información aprendida por el surrogate, semillas validadas y restricciones de manifold para concentrar la búsqueda en regiones fértiles del espacio. La diferencia entre una tasa histórica del $2.1\%$ por encima de $R^2 \geq 0.85$ y del $60\%$ en el GA restringido al manifold cuantifica el valor del enfoque asistido frente a una exploración no dirigida.

% --------------------------------------------------
\subsection{Cuencas de Óptimos Independientes}\label{subsec:cuencas}

Una auditoría del conjunto de datos reveló que la región $R^2 \geq 0.90$ no es un único óptimo sino tres cuencas geométricamente independientes, separadas por distancias euclidianas $\ell_2 \approx 0.5$ en el espacio de diseño (Tabla~\ref{tab:cuencas}). El optimizador siempre había convergido a una sola de ellas (la de \texttt{sim\_627}) porque ahí se ubicaban sus semillas. Las otras dos, incluyendo el máximo absoluto del conjunto de datos, nunca se habían explorado como semillas.
...
(Sección~\ref{subsec:hill_climbing}): $50$ perturbaciones gaussianas de $\sigma = 5$~nm por cuenca, validadas individualmente. Las cuencas 042 y 721 se sembraron desde su máximo del diseño de experimentos (\texttt{sim\_042} y \texttt{sim\_721}, Tabla~\ref{tab:cuencas}); la cuenca 627 se sembró desde \texttt{mvp\_c09} ($R^2_{\text{real}} = 0.9025$), el mejor candidato que el algoritmo genético había confirmado en ella durante la iteración~4, con el fin de refinar el producto del pipeline en lugar del punto histórico \texttt{sim\_627}. La Tabla~\ref{tab:hill_climbing} resume los resultados, que revelan una diferencia cualitativa fundamental entre cuencas.
...
La cuenca 627 muestra mayor tolerancia relativa; las cuencas 042 y 721 colapsan con mayor frecuencia.}
...
\begin{figure}[H]
    ...
    \caption{Distribución de $R^2_{\text{real}}$ bajo perturbaciones de $\sigma=5$~nm por cuenca. La cuenca amplia (627, verde) mantiene una distribución menos degradada que las cuencas aguja; las cuencas 042 y 721 (rojas) se dispersan muy por debajo del colapso de enfoque ($R^2 < 0.70$). El triángulo marca la media.}
...
\begin{itemize}
    \item \textbf{Cuenca amplia (627)}: presenta mayor tolerancia relativa a
    perturbaciones de $\pm 5$~nm y el hill-climbing incluso mejoró su semilla
    \texttt{mvp\_c09}, elevando el $R^2$ de $0.9025$ a $0.9256$ (\texttt{hc627\_036}). Su $R^2$ medio bajo perturbación es $0.73$ y produjo tres geometrías con $R^2 \geq 0.90$.
...
En términos de ingeniería, una tolerancia de fabricación litográfica del orden de $\pm 5$~nm degrada severamente el enfoque en las cuencas aguja, mientras que la cuenca 627 muestra una robustez relativa mayor. El valor del pipeline ML no fue meramente cruzar el umbral $R^2 \geq 0.90$, sino localizar una región del paisaje con mayor probabilidad de retener desempeño útil bajo perturbaciones nanométricas que los máximos puntuales hallados por el muestreo aleatorio del diseño de experimentos. La Figura~\ref{fig:aguja_meseta} sintetiza el contraste de forma esquemática: una cuenca aguja alcanza un máximo más alto pero colapsa dentro de la banda de tolerancia de $\pm5$~nm, mientras que la meseta sacrifica algo de pico a cambio de una degradación menos abrupta alrededor del óptimo.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.74\textwidth]{Figuras/fig_aguja_meseta.png}
    \caption{Esquema del hallazgo central, anclado a los valores reales. La cuenca \emph{aguja} (042/721) alcanza un $R^2$ máximo más alto ($0.958$) pero su perfil es tan estrecho que la tolerancia de fabricación de $\pm5$~nm (banda gris) lo hunde por debajo del colapso de enfoque; la cuenca \emph{meseta} (627, máximo $0.926$) presenta una degradación menos abrupta y conserva más candidatos útiles bajo la misma escala de perturbación. La robustez relativa, no el máximo puntual, es lo
...
La geometría recomendada como resultado del proyecto es \texttt{hc627\_036}, con $R^2_{\text{real}} = 0.9256$, por combinar alto rendimiento y robustez relativa dentro de la cuenca 627. Se acompaña de dos candidatos adicionales de la misma cuenca, \texttt{hc627\_008} ($0.9134$) y \texttt{hc627\_043} ($0.9014$), que ofrecen alternativas confirmadas por FDTD con $R^2 \geq 0.90$. La geometría se caracteriza por $c_{2c} = 0.300\;\mu$m y un perfil de anchos variable con media ${\sim}0.088\;\mu$m. El conjunto completo de candidatos ordenados se entrega en el repositorio de código (\texttt{results/final/}). La Figura~\ref{fig:perfil_hc627} muestra su perfil de anchos y la Figura~\ref{fig:campo_hc627} el perfil de intensidad resultante en el plano focal.
...
El perfil de anchos (Figura~\ref{fig:perfil_hc627}) no es la parábola suave de una lente convergente clásica, sino un patrón oscilatorio cuasi-periódico, en el que los anchos alternan entre ${\sim}30$ y ${\sim}150$~nm a lo largo de unos cuatro lóbulos del arreglo. Es el tipo de configuración no intuitiva que un diseño analítico guiado por la intuición de lente difícilmente propondría y que el diseño inverso asistido por aprendizaje sí localiza: el enfoque emerge de una modulación distribuida y compleja de la fase, no de un gradiente monotónico de anchos.
...
El contraste sitúa este trabajo frente al estado del arte en varios planos. Frente al diseño inverso directo, en el que una red aprende el mapeo respuesta~$\rightarrow$~geometría y debe sortear la no-unicidad del problema, como en la arquitectura en tándem de~\cite{liu2018tandem}, aquí se emplea un subrogado \textit{directo} (geometría $\rightarrow$ respuesta) acoplado a un optimizador, lo que evita la patología de no-unicidad a costa de delegar la búsqueda al algoritmo genético. A diferencia de los modelos que predicen solo un escalar (un parámetro geométrico o una figura de mérito, como en~\cite{maia2025inverse}), este trabajo predice el campo óptico focal completo y lo complementa con una cabeza escalar supervisada para $\hat{R}^2_{\text{pred}}$, decisión que fue esencial para evitar el desbalance que estancó el primer enfoque y para no depender de la propagación del campo predicho, sensible a errores de fase (Sección~\ref{sec:modelado}). Por último, aunque comparte con~\cite{gao2025surrogate} la filosofía de acoplar un subrogado con un optimizador evolutivo, aporta dos elementos exigidos por la escasez de datos ($\sim$1{,}100 simulaciones en 102 dimensiones): la restricción al manifold de geometrías validadas, sin la cual el optimizador explota las regiones donde el subrogado extrapola, y la caracterización de robustez de las cuencas, que reveló el hallazgo central aguja/amplia (Sección~\ref{subsec:aguja_amplia}).
...
\begin{enumerate}
    ...
    \emph{no son representables} en el espacio de entrada del modelo sin redefinir su parametrización. Las geometrías completas sí se conservan en los archivos de diseño de experimentos, pero no pueden incorporarse al modelo actual sin una representación de entrada compatible con longitud variable.
