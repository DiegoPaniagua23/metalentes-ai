...
\begin{itemize}
    ...
    \item \textbf{Optimización (Objetivo 3):} se desarrolló un pipeline de diseño inverso (algoritmo genético restringido al manifold con aptitud combinada, más refinamiento por hill-climbing FDTD) que halló geometrías que cumplen el criterio de éxito y, en conjunto, desplazó la calidad de los candidatos muy por encima del conjunto histórico consolidado ($R^2_{\text{real}}$ medio de ${\sim}0.79$ frente a $0.24$ del dataset consolidado; Sección~\ref{subsec:convergencia}).
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Reformulación del objetivo de aprendizaje.} El hallazgo metodológico central fue que predecir el campo óptico focal (objetivo denso), complementado con una cabeza escalar supervisada para $\hat{R}^2_{\text{pred}}$, evita depender exclusivamente del escalar de calidad $R^2$ como objetivo escaso y desbalanceado. El criterio de éxito deja de ser el objetivo único de entrenamiento, se usa para priorizar candidatos mediante el surrogate y se confirma finalmente por FDTD.

    \item \textbf{Necesidad de restringir la búsqueda al manifold.} Un optimizador libre sobre un surrogate imperfecto explota sus debilidades y converge a regiones no vistas donde el modelo sobreestima. Restringir el GA a vecindarios de geometrías validadas fue indispensable para producir candidatos físicamente fiables.

    \item \textbf{Cuencas ``aguja'' frente a ``amplia''.} El paisaje de diseño contiene óptimos cualitativamente distintos: máximos puntuales muy sensibles a perturbaciones de fabricación (``aguja'', p.ej.\ $R^2 = 0.9579$ que colapsa bajo $\pm 5$~nm) y cuencas relativamente más robustas bajo perturbaciones de esa escala. El pipeline ML localizó la cuenca 627, de mayor valor de ingeniería que el máximo puntual del muestreo aleatorio por su menor degradación local.

    \item \textbf{El estancamiento como propiedad del paisaje.} El límite $[0.85, 0.91]$ observado en las iteraciones no se explica solo por un defecto del modelo: persistió a través de tres surrogates, tres radios de búsqueda y distintas semillas, y el hill-climbing por FDTD directa es consistente con que refleja la estructura local del problema físico en las cuencas exploradas.
\end{enumerate}
