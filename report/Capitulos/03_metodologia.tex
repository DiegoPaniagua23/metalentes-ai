...
\begin{figure}[H]
    \centering
    \colorlet{goldau}{yellow!60!orange!80}
    \colorlet{goldstroke}{brown!80!black}
    \colorlet{substrate}{blue!20!white}
    \colorlet{substroke}{blue!60}
    \colorlet{lightgreen}{green!50!teal}
    \colorlet{annoRed}{red!70!black}
    \colorlet{annoBlue}{blue!70!black}
    \input{Figuras/arreglo_inc.tikz}
    ...
    rendija $w$ (\texttt{widths}) y la separación centro a centro $c$ (\texttt{c2c\_d}).}
    \label{fig:arreglo}
\end{figure}
...
\begin{enumerate}
    ...
    \item $0.150 \leq c_j \leq 0.300\;\mu$m \quad (separación centro a centro entre rendijas adyacentes).
    \item $c_j - \tfrac{1}{2}(w_j + w_{j+1}) \geq 0.020\;\mu$m \quad (pared mínima de oro entre rendijas).
    \item $0.1 \leq w_i/c \leq 0.5$ \quad (factor de llenado en el régimen de resonancia óptima para EOT).
\end{enumerate}
...
De este modo, el dataset conserva la información de todas las simulaciones, incluidas aquellas que no producen enfoque satisfactorio, y no solo la escasa región de alto $R^2$. Esta representación densa no elimina las limitaciones asociadas al tamaño del conjunto, pero permite aprovechar información que el objetivo escalar dejaba sin uso.

\subsubsection{Estructuración del Dataset}

De cada archivo JSON se extraen el identificador de la simulación, el vector geométrico $\mathbf{x} \in \mathbb{R}^{102}$, compuesto por $101$ anchos y una separación centro a centro $c$ constante por metalente, la amplitud y fase del campo focal re-muestreadas a $1{,}024$ puntos, y el coeficiente $R^2$ asociado al perfil de intensidad.
...
El dataset oficial consolida $1{,}110$ simulaciones: $1{,}000$ provienen del diseño de experimentos inicial (\texttt{sim\_XXX}), $90$ corresponden al ciclo de mejora inicial y $20$ a geometrías confirmadas durante las iteraciones de diseño inverso. Para preservar representación de la escasa región de alto $R^2$ en cada partición, se aplicó estratificación por bandas de $R^2$ ($<0.30$, $[0.30,0.70)$, $[0.70,0.85)$, $\geq 0.85$) con semilla fija ($\texttt{seed}=42$) y proporciones $70\%/15\%/15\%$.
