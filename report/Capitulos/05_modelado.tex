...
Esta sección describe el diseño y entrenamiento del modelo subrogado que aproxima la relación entre la geometría de la metalente y su respuesta óptica, sin ejecutar simulaciones FDTD. El desarrollo del subrogado atravesó dos paradigmas sucesivos: un primer enfoque que predecía directamente la métrica escalar de calidad ($R^2$) y que fue inviable por la naturaleza del problema, y un segundo enfoque, el adoptado finalmente, que predice el campo óptico focal completo. Se documentan ambos porque la transición entre ellos es uno de los aprendizajes centrales del proyecto.
...
Se implementaron dos mitigadores. El primero fue un umbral suave, que redefinía la etiqueta Valid del clasificador como $R^2 \geq 0.70$ (no $0.90$), con lo que los positivos pasaron de ${\sim}3$ a ${\sim}30$ y el AUC subió de ${\sim}0.50$ a $0.74$. El segundo fue una ponderación graduada de muestras por banda de $R^2$ (pesos crecientes hacia la zona alta), para guiar al regresor hacia la región de alto rendimiento. Estas mejoras se integraron además en un ciclo de aprendizaje activo con \textit{Query by Committee} que concentraba el presupuesto de simulaciones FDTD en las geometrías de mayor incertidumbre (detallado en la Sección~\ref{sec:optimizacion}).

A pesar de las mejoras, dos ejecuciones completas del ciclo activo (${\sim}180$ simulaciones FDTD adicionales, de las cuales $90$ con geometría recuperable se integraron luego al dataset consolidado, Sección~\ref{subsec:consolidacion}) no lograron convergencia: el $R^2$ de validación permaneció estancado en el rango $0.12$--$0.15$ sin tendencia, y apareció una brecha sistemática entre el $R^2$ predicho por el surrogate y el $R^2$ real confirmado por FDTD (el optimizador convergía a geometrías con $R^2$ predicho alto pero $R^2$ real muy inferior). El desbalance no era, entonces, un defecto corregible del modelo, sino una propiedad estructural de usar un escalar binario como objetivo: la región $R^2 \geq 0.90$ ocupa una fracción ínfima del espacio de diseño, y ninguna técnica de re-ponderación crea señal donde casi no hay
ejemplos.

% --------------------------------------------------
\subsection{Re-formulación del Problema de Aprendizaje}\label{subsec:reformulacion}

El estancamiento motivó una auditoría y re-formulación del enfoque. El simulador FDTD no produce un escalar, sino un campo óptico completo en el plano focal, del que el $R^2$ es solo una proyección. Aprender a predecir el campo, un objetivo denso y continuo, en lugar de depender exclusivamente del escalar, elimina el desbalance estructural: todas las simulaciones, enfocadoras o no, contribuyen con información sobre el mapeo geometría $\rightarrow$ respuesta óptica. En el modelo final, el criterio de éxito $R^2 \geq 0.90$ se conserva además mediante una cabeza escalar supervisada, incorporada porque derivarlo por propagación del campo predicho no fue suficientemente estable.
...
\noindent representado por dos vectores re-muestreados a longitud fija $L = 1024$: la amplitud $|E|(y) \in \mathbb{R}^{+}$ (normalizada al pico de cada simulación) y la fase desenvuelta $\Phi(y) \in \mathbb{R}$ (centrada por su mediana, dado que el desfase global no afecta la formación del foco). El vector de entrada del modelo es $\mathbf{x} \in \mathbb{R}^{102}$ (101 anchos de rendija más la separación centro a centro $c_{2c}$), una simplificación de las 201 variables que describirían la geometría completa de una metalente de $101$ rendijas (101 anchos y 100 separaciones independientes), posible porque en el diseño de experimentos la separación centro a centro se mantiene constante por metalente.
...
\begin{figure}[H]
    \centering
    \resizebox{\textwidth}{!}{\input{Figuras/mlp_arquitectura.tikz}}
...
\subsubsection[Inclusión de la Cabeza Escalar R2 Predicha]{Inclusión de la Cabeza Escalar $\hat{R}^2_{\text{pred}}$}\label{subsubsec:r2head}

Idealmente, el criterio de éxito se calcularía propagando el campo predicho a una región longitudinal y ajustando la parábola, como hace el extractor de \textit{features} del simulador. Se implementó dicha propagación mediante el método del espectro angular (\textit{Angular Spectrum Method}), validándola con campos reales (correlación $0.84$ con el $R^2$ real del archivo HDF5). Sin embargo, al alimentarla con el campo \textit{predicho} por el subrogado, la correlación cae a ${\sim}0.20$: la propagación FFT amplifica los errores de fase del subrogado. De ahí que se añadiera $\hat{R}^2_{\text{pred}}$ como salida escalar supervisada directamente, evitando la propagación en la trayectoria de optimización.

\subsubsection{Ponderación por Muestra}

Para reforzar el aprendizaje en la región de alto rendimiento, se asignó una ponderación por muestra según la banda de $R^2$ y el origen del dato. Las simulaciones de las iteraciones de confirmación con $R^2 \geq 0.90$ reciben el peso máximo, las de $[0.85, 0.90)$ un peso intermedio-alto, y la masa central ($R^2$ bajo) peso unitario. A diferencia del primer enfoque, donde corregía un desbalance de clases, aquí la ponderación enfatiza la fidelidad del campo predicho donde más importa, pues ya no hay clasificación.
...
Las curvas revelan dos rasgos coherentes con la naturaleza del problema. Por un lado, el margen de generalización es moderado: la pérdida de entrenamiento desciende muy por debajo de la de validación, pero esta última no aumenta tras su mínimo, sino que se estabiliza. El \textit{dropout} y el \textit{early stopping} en la época $47$ contienen el sobreajuste sin necesidad de regularización adicional, un resultado esperable dado el tamaño reducido del dataset frente a la dimensión de la salida. Por otro, la descomposición por componente (panel b) confirma lo anticipado en el EDA: la fase es la fuente dominante e inestable de error, y sus picos corresponden a la minoría de geometrías con frentes de onda de gran excursión, mientras que la amplitud y $\hat{R}^2_{\text{pred}}$ se aprenden de forma estable. La Tabla~\ref{tab:surrogate_metrics}
...
En la zona de alto rendimiento ($R^2_{\text{real}} \geq 0.7$) la correlación se debilita e incluso se invierte: el surrogate sobreestima en esa región. Por eso un valor $\hat{R}^2_{\text{pred}}$ elevado no garantiza un $R^2$ real alto, y la confirmación por FDTD sigue siendo indispensable para verificar el criterio de éxito. Esta limitación condiciona el diseño del optimizador (Sección~\ref{sec:optimizacion}), que no puede confiar ciegamente en el escalar predicho y debe restringirse a vecindarios del manifold de geometrías ya validadas.
